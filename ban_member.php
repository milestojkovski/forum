...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($_POST['sure'] == 'Yes') { // Delete the record.
		...
		// Make the query:
		$q = $sql = "UPDATE users SET ban='1' WHERE id=$id LIMIT 1";
		$r = @mysqli_query ($dbc, $q);
		if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.

			// Print a message:
			echo '<p>The member has been banned.</p>';	

		} else { // If the query did not run OK.
			echo '<p class="error">The member could not be banned due to a system error.</p>'; // Public message.
			echo '<p>' . mysqli_error($dbc) . '<br />Query: ' . $q . '</p>'; // Debugging message.
		}
	
	} else { // No confirmation of deletion.
		echo '<p>The member has NOT been banned.</p>';	
	}
	...
	if (mysqli_num_rows($r) == 1) { // Valid memeber ID, show the form.
		...
		echo "<h3>Name: $row[0] <br></h3>
                      <h3> Email: $row[1]</h3>
		Are you sure you want to bann this member?";
